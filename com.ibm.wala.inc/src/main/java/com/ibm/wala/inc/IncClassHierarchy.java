...
public class IncClassHierarchy extends ClassHierarchy {
	...
	public IncClassHierarchy(IncJavaSourceAnalysisScope scope, ClassLoaderFactory factory,
	...
	/**
	 * update a list of changed files. New classes can be added or existing classes
	 * will be updated.
	 *
	 * @param files
	 * @return
	...
	private void updateReferences(IClass klass) {
		for(MethodReference ref: MethodReference.find(klass.getName()))
		{
	...
	public void printAllClasses() {
		...
		while (it.hasNext()) {
			...
			if (k instanceof JavaClass) {
				...
				System.out.println("VersionedClassHierarchy - classs in vcha " + klass.getSourceURL().toString());
			}
		}
	}
}
