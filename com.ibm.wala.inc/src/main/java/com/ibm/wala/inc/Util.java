...
public class Util {
  ...
  //				if (m != null) {
  //					System.out.println(Util.class + "- Found main method in " + ((JavaClass)
  // klass).getSourceURL());
  ...
  public static Iterable<Entrypoint> makeMainEntrypoints(IClassHierarchy cha) {
    ...
    for (IClass klass : cha) {
      if (klass.getClassLoader().getName().toString().equals("Application")
          || klass.getClassLoader().getName().toString().equals("Source")
          || klass.getClassLoader().getName().toString().startsWith("Incremental")) {
        System.out.println(
            klass.getName().toString()
                + " from "
                + klass.getClassLoader().getName().toString()
                + " loader");
        IClass superKlass = klass.getSuperclass();
        System.out.println(
            klass.getName().toString()
                + " has super class "
                + superKlass.getName().toString()
                + " from "
                + superKlass.getClassLoader().getName()
                + " loader\n");
