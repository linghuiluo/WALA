...
public class VersionedClassHierarchyTest {
	...
	public void prepare() {
		try {
			File projectSourcePath = new File(
					System.getProperty("user.dir") + "/src/test/resources/CGTestProject1/src");
	...
	public void inc() {
		File file = new File(System.getProperty("user.dir") + "/src/test/resources/CGTestProject2/src/C.java");
	...
	public void test() {
		...
		ServerAnalysis analysis = new ServerAnalysis() {
			...
			public void analyze(Collection<Module> files, MagpieServer server) {
				...
				IncSourceLoaderImpl incLoader = new IncSourceLoaderImpl(IncJavaSourceAnalysisScope.INCREMENTAL,
						vcha.getLoader(ClassLoaderReference.Application), vcha);
			...
			public void prepare(IProjectService ps) {
				try {
					...
					Iterable<? extends Entrypoint> entrypoints = com.ibm.wala.ipa.callgraph.impl.Util
							.makeMainEntrypoints(scope.getApplicationLoader(), vcha);
					options.setEntrypoints(entrypoints);
					options.setReflectionOptions(ReflectionOptions.NONE);
					IAnalysisCacheView cache = new AnalysisCacheImpl(AstIRFactory.makeDefaultFactory());
					builder = (SSAPropagationCallGraphBuilder) new ZeroCFABuilderFactory().make(options, cache, vcha,
							scope);
